...
export function buildUpcomingEvents() {
  ...
  for (const ev of EXTRA_EVENTS) {
    if (dayStrs.has(ev.date)) {
      events.push({
        // `ev.id` pozwala przypiąć wydarzenie do istniejącego klucza zapisów
        // (np. gdy trening zamieniamy na sparing, ale chcemy zachować dotychczasowe
        // odpowiedzi). Bez tego pola id ma standardowy format "typ-RRRR-MM-DD".
...
const TACTIC_BOARD_IMAGE = "assets/img/taktyka.jpg?v=59";
